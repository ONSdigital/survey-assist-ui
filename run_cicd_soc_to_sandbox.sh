...
gcloud beta builds submit . --config=cicd/cloudbuild_dev_and_sandbox.yaml \
	...
	--substitutions=SHORT_SHA=$GIT_SHA,_BACKEND_API_URL=$API_URL,_BACKEND_SA_EMAIL=$API_SA_EMAIL,_GAR_IMAGE=$GAR_IMAGE,_PROXY_API_URL=$PROXY_API_URL,_GUNICORN_WORKERS=6,_TARGET_PROJECT_ID=$PROJECT_ID,_VERIFY_API_URL=$OTP_URL \
	--region $REGION
