...
OTP_URL=$(echo $sandbox_config | python3 -c "import sys, json; print(json.load(sys.stdin)['cr-otp-api-url'])")
API_URL=$(echo $sandbox_config | python3 -c "import sys, json; print(json.load(sys.stdin)['cr-api-url'])")
API_SA_EMAIL=$(echo $sandbox_config | python3 -c "import sys, json; print(json.load(sys.stdin)['api-sa-email'])")
...
gcloud beta builds submit . --config=cicd/cloudbuild_dev_and_sandbox.yaml \
	...
	--substitutions=SHORT_SHA=$GIT_SHA,_BACKEND_API_URL=$API_URL,_BACKEND_SA_EMAIL=$API_SA_EMAIL,_GAR_IMAGE="europe-west2-docker.pkg.dev/survey-assist-sandbox/survey-assist-ui/survey-assist-ui",_GUNICORN_WORKERS=6,_TARGET_PROJECT_ID=$PROJECT_ID,_UI_SA_EMAIL="survey-ui-cloud-run@survey-assist-sandbox.iam.gserviceaccount.com",_VERIFY_API_URL=$OTP_URL \
	--region $REGION
