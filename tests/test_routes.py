...
@pytest.mark.route
def test_survey_assist_with_disabled(client) -> None:
    """Tests that the survey assist route returns a 302 response
    when the survey assist feature is disabled.
    ...
    route_text = "survey-assist"

    with patch.object(app, "survey_assist", {"enabled": False}):
        response = client.get("/" + route_text)
        assert (
            response.status_code == HTTPStatus.FOUND
        ), "{route_text} should redirect when Survey Assist Disabled"
        assert response.headers["Location"].endswith(
            "/survey"
        ), "Expected redirect to /survey when Survey Assist is disabled"


@pytest.mark.route
def test_survey_assist_without_interactions(client) -> None:
    """Tests that the survey assist route returns a 302 response
    when the survey assist feature is disabled.
    ...
    with patch.object(app, "survey_assist", {"enabled": True, "interactions": []}):
        response = client.get("/" + route_text)
        assert (
            response.status_code == HTTPStatus.FOUND
        ), "{route_text} should redirect when Survey Assist Disabled"
        assert response.headers["Location"].endswith(
            "/survey"
        ), "Expected redirect to /survey when Survey Assist is enabed but interactions are empty"
